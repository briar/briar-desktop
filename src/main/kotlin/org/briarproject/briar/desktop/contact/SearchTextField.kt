...
fun SearchTextField(searchValue: String, onValueChange: (String) -> Unit, onContactAdd: () -> Unit) {
    Surface(modifier = Modifier.fillMaxSize(), color = TopAppBar, contentColor = MaterialTheme.colors.onTopAppBar) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            TextField(
                value = searchValue,
                onValueChange = onValueChange,
                singleLine = true,
                textStyle = TextStyle(fontSize = 16.sp, color = MaterialTheme.colors.onTopAppBar),
                placeholder = { Text(i18n("contacts.search.title"), color = MaterialTheme.colors.onTopAppBar) },
                shape = RoundedCornerShape(0.dp),
                leadingIcon = {
                    val padding = Modifier.padding(top = 8.dp, bottom = 8.dp, start = 12.dp)
                    Icon(Icons.Filled.Search, i18n("access.contacts.search"), padding, tint = MaterialTheme.colors.onTopAppBar)
                },
                modifier = Modifier.width(COLUMN_WIDTH - 56.dp)
            )
            IconButton(
